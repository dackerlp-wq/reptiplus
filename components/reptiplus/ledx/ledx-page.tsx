...
import { LedxInquiryForm, type RadaConfig } from "./inquiry-form";

const FLOOD: RadaConfig = {
  id: "pop-flood",
  rada: "Flood Light",
  models: ["Flood Light 50 W", "Flood Light 100 W", "Flood Light 150 W", "Flood Light 200 W", "Flood Light Linear 100 W", "Flood Light Linear 150 W"],
  cct: ["2700 K (teplá)", "3000 K", "5000 K", "6000 K", "6500 K (denní)"],
  uhel: ["15°", "40°", "60°", "90°"],
};
const PH1: RadaConfig = {
  id: "pop-phoenix1",
  rada: "Phoenix 1",
  models: ["Phoenix 1 · 120 W", "Phoenix 1 · 240 W", "Phoenix 1 · 360 W", "Phoenix 1 · 480 W", "Sestava 480+ W"],
  cct: ["3000 K", "4000 K (neutrální)", "5000 K", "6500 K (denní)"],
  uhel: ["20°", "40°", "60°", "90°", "asymetrický (H140×V100)"],
};
const GROW: RadaConfig = {
  id: "pop-grow",
  rada: "Phoenix Grow",
  models: ["Phoenix Grow · 120 W", "Phoenix Grow · 240 W", "Phoenix Grow · 360 W", "Phoenix Grow · 480 W", "Sestava až 1440 W"],
  cct: { fixed: "4200 K · Grow spektrum" },
  uhel: ["20°", "40°", "60°", "90°", "asymetrický (H140×V100)"],
};

type View = "home" | "flood" | "phoenix1" | "grow";
const DETAILS: View[] = ["flood", "phoenix1", "grow"];

export function LedxPage() {
  const [view, setView] = useState<View>("home");

  useEffect(() => {
    const apply = () => {
      const h = location.hash.replace("#", "");
      if ((DETAILS as string[]).includes(h)) {
        setView(h as View);
        window.scrollTo(0, 0);
      } else {
        setView("home");
    ...
    <div className="ledx">
      ...
      <div hidden={view !== "home"}>
        ...
        <section className="ticker"><div className="shell"><div className="trow">
          <div className="stat"><div className="n">CRI až <b>95</b></div><div className="l">Věrné podání barev</div></div>
          <div className="stat"><div className="n">až <b>74 000</b> lm</div><div className="l">Světelný tok (480 W)</div></div>
          <div className="stat"><div className="n">IP<b>65</b>/<b>66</b></div><div className="l">Krytí do náročných provozů</div></div>
          <div className="stat"><div className="n">až <b>150</b> lm/W</div><div className="l">Světelná účinnost</div></div>
        </div></div></section>

        <section className="intro"><div className="shell">
          <p className="kicker gold">Proč LEDX</p>
          <p className="lead">Ne katalogové svítidlo. <span className="hl">Řešení navržené k projektu</span> — barva světla, úhel vyzařování i výkon podle konkrétního prostoru.</p>
        ...
        <section className="cta"><div className="shell"><div className="box"><div className="in">
          <p className="kicker">Objednávka &amp; poradenství</p>
          <h2>Poradíme s výběrem a připravíme nabídku.</h2>
          <p>Jako výhradní dodavatel LEDX pro ČR dodáváme svítidla na objednávku. Vyberte řadu a vyplňte krátkou poptávku — doporučíme výkon i barvu světla a připravíme nabídku. Nezávazně.</p>
          <div className="cta-row"><a className="btn primary" href="#rady">Vybrat řadu a poptat <Arrow /></a></div>
        </div></div></div></section>
      </div>
      ...
      <DetailView show={view === "flood"} no="ŘADA 01 · LEDX" title="Flood Light" sub="Univerzální reflektory"
        ...
        params={[["Barevná teplota", "2700–6500 K"], ["Úhel vyzařování", "15/40/60/90°"], ["Podání barev", "CRI 85–90"], ["Účinnost", ">115 lm/W"], ["Krytí / třída", "IP66 · I · IK08"], ["Napájení", "90–305 V · 50/60 Hz"], ["Provozní teplota", "−40 až +50 °C"], ["Materiál", "hliník · PMMA · nerez"], ["Stmívání", "0–10V / DALI / Zigbee"], ["Energ. třída", "E"], ["Životnost", "L80/B20 · 50 000 h"], ["Certifikáty", "CE · RoHS"]]}
        usesTitle="Kde se hodí" uses={["Terária & expozice", "Tropické pavilony", "Akvária", "Výběhy zvířat", "Zoo"]}
        config={FLOOD} />
      ...
      <DetailView show={view === "phoenix1"} no="ŘADA 02 · LEDX" title="Phoenix 1" sub="Univerzální výkonová řada"
        ...
        params={[["Barevná teplota", "3000–6500 K"], ["Úhel vyzařování", "20–90° / H140×V100"], ["Podání barev", "CRI 85–95"], ["Účinnost", "~150 lm/W"], ["Krytí / třída", "IP65 · I · IK08"], ["Chlazení", "pasivní"], ["Provozní teplota", "−40 až +55 °C"], ["Materiál", "hliník · PMMA · nerez"], ["Stmívání", "0–10V / DALI / Zigbee"], ["Energ. třída", "C"], ["Životnost", "L80/B10 · 50 000 h"], ["Certifikáty", "CE · RoHS"]]}
        usesTitle="Jedna lampa, pět aplikací" uses={["Reflektorové (flood)", "Pouliční", "Tunelové", "Stožárové", "High-bay", "Expozice primátů", "Tropické pavilony"]}
        config={PH1} />
      ...
      <DetailView show={view === "grow"} no="ŘADA 03 · LEDX" title="Phoenix Grow" sub="Spektrum pro růst rostlin"
        ...
        params={[["Barevná teplota", "4200 K"], ["Spektrum", "380–780 nm"], ["Podání barev", "CRI 95 · TLCI >95 %"], ["Účinnost", ">135 lm/W"], ["Úhel vyzařování", "20–90° / H140×V100"], ["Krytí / třída", "IP65 · I · IK08"], ["Chlazení", "pasivní"], ["Provozní teplota", "−40 až +55 °C"], ["Stmívání", "0–10V / DALI / Zigbee"], ["Energ. třída", "C"], ["Životnost", "L70/B20 · 30 000 h"], ["Certifikáty", "CE · RoHS"]]}
        usesTitle="Kde se hodí" uses={["Skleníky", "Tropické pavilony", "Pěstírny a množárny", "Vegetační ostrůvky", "Odchovny rostlin"]}
        config={GROW} />
    </div>
  );
}

function LineCard({ no, title, sub, tagline, desc, pills, img, href, flip }: {
  no: string; title: string; sub: string; tagline: string; desc: string; pills: string[]; img: string; href: string; flip?: boolean;
}) {
  return (
    <section className={`line${flip ? " flip" : ""}`}><div className="shell"><div className="lgrid">
      <div>
        <div className="line-no">{no}</div>
        <h2 className="line-h">{title}<span>{sub}</span></h2>
        <p className="tagline">{tagline}</p>
        <p className="desc">{desc}</p>
        <div className="hi-specs">{pills.map((p) => <span key={p} className="hipill">{p}</span>)}</div>
        <div className="cta-row"><a className="btn primary" href={href}>Detail řady
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M13 6l6 6-6 6" /></svg></a></div>
      </div>
      <div><a href={href}><div className="plinth"><span className="tag">{title}</span><img src={img} alt={`LEDX ${title}`} /></div></a></div>
    </div></div></section>
  );
}
  ...
  return (
    <div hidden={!show}>
      ...
      <section className="dhero"><div className="shell"><div className="dhero-grid">
        <div>
          <div className="line-no">{no}</div>
          <h1 className="dtitle">{title}<span>{sub}</span></h1>
          <div className="badge-row"><span className="pill-order">Na objednávku · výhradní dovoz pro ČR</span></div>
          <p className="dlead">{lead}</p>
          <div className="hi-specs">{pills.map((p) => <span key={p} className="hipill">{p}</span>)}</div>
      ...
      <section className="dsection"><div className="shell">
        <h2 className="dh2">Modely a varianty</h2>
        <p className="sec-sub">{modelsNote}</p>
        <div className="tablewrap"><table>
          <thead><tr><th>Model</th><th>Výkon</th><th>Světelný tok</th><th>Rozměry (D×Š×V)</th><th>Hmotnost</th></tr></thead>
          <tbody>{rows.map((r) => <tr key={r[0]}><th>{r[0]}</th>{r.slice(1).map((c, i) => <td key={i}>{c}</td>)}</tr>)}</tbody>
        </table></div>
      </div></section>

      <section className="dsection alt"><div className="shell">
        <h2 className="dh2">Společné parametry</h2>
        <div className="paramgrid">{params.map(([k, v]) => <div key={k} className="param"><div className="k">{k}</div><div className="v">{v}</div></div>)}</div>
      </div></section>

      <section className="dsection"><div className="shell">
        <h2 className="dh2">{usesTitle}</h2>
        <div className="tagcloud">{uses.map((u) => <span key={u}>{u}</span>)}</div>
      </div></section>

      <section className="dsection alt" id={config.id}><div className="shell"><div className="qwrap">
        <p className="kicker gold">Nezávazná poptávka</p>
        <h2 className="dh2">Poptat řadu {title}</h2>
        <p className="sec-sub">Světla dodáváme na objednávku — vyberte konfiguraci a připravíme nabídku na míru. Ozveme se do 2 pracovních dnů.</p>
        <LedxInquiryForm config={config} />
      </div></div></section>
    </div>
  );
}
